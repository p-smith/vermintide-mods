...
dofile("scripts/mods/luck_and_dupe/luck_and_dupe_gui")
...
local enabled_luck = false
local enabled_dupe = false

local table = {
	has_item = function(tbl, item)
...
local function create_chest_icon(unit, pickup_name)
	...
	if POSITION_LOOKUP[player.player_unit] then
		...
		if distance > 0.5 and distance < 6.5 and depth < 1 then
			...
			mod:pcall(function()
				mod.gui.draw_icon("icon_trophy_luckstone_01_01", pos, Color(125, 255, 255, 255), size, "gui_item_icons_atlas")
			end)
		end
	end
end
...
local function create_dupe_icon(unit, pickup_name)
	...
	if POSITION_LOOKUP[unit] and POSITION_LOOKUP[player.player_unit] then
		...
		if distance > 0.5 and distance < 6.5 and depth < 1 then
			...
			mod:pcall(function()
				mod.gui.draw_icon("icon_trophy_ornamented_scroll_case_01_01", pos, Color(100, 255, 255, 255), size, "gui_item_icons_atlas")
			end)
		end
	end
end
...
mod.on_all_mods_loaded = function()
	mod.gui.init()
    enabled_luck = mod:get("show_luck")
    enabled_dupe = mod:get("show_dupe")
end
