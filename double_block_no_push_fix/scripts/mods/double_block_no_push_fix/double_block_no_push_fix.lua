local mod = get_mod("double_block_no_push_fix")

local AllMeleeWeaponTemplates = {}

local function enter_function_with_delay_hook(attacker_unit, input_extension, remaining_time)
...
for _, template in pairs(AllMeleeWeaponTemplates) do
	mod:hook_origin(Weapons[template].actions.action_two.default, "enter_function", enter_function_with_delay_hook)
end
...
mod:hook(PlayerInputExtension, "update", function(func, self, unit, input, dt, context, t)
